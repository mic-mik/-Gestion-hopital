...
const ListDepartement = () => {
  ...
  useEffect(() => {
    const fetchDepartements = async () => {
      try {
        ...
        console.error(
          "Il y a eu une erreur lors de la récupération des départements !",
          error
        );
      }
    };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setDepartements(
        departements.filter((departement) => departement.IdDepartement !== id)
      );
      alert("Département supprimé avec succès");
    } catch (error) {
      console.error(
        "Il y a eu une erreur lors de la suppression du département !",
        error
      );
    }
  };
              ...
              <tr key={departement.IdDepartement}>
                ...
                {/* <td>
                  {departement.Lit && departement.Lit.length > 0
                    ? departement.Lit.map((lit) => lit.Id).join(", ")
                    : "Aucun lit associé"}
                </td> */}
                <td>
                  <button onClick={() => handleView(departement.IdDepartement)}>
                    Voir
                  </button>
                  <button onClick={() => handleEdit(departement.IdDepartement)}>
                    Modifier
                  </button>
                  <button
